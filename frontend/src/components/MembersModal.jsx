...
export default function MembersModal({ isOpen, onClose, event, user }) {
  ...
  const canEditMembers = !hasPassed && [roles.PRESIDENTE, roles.COORDINADOR].includes(user.role);
      ...
      <div className="bg-white shadow-lg rounded-xl p-6 w-full max-w-md relative">
        ...
        {hasPassed && (
          <div className="text-center text-sm text-gray-500 mb-2">
            Este evento ya ocurrió. No se pueden modificar los miembros.
          </div>
        )}

        {!canEditMembers && !hasPassed && (
          <div className="text-center text-sm text-gray-500 mb-2">
            Solo los roles PRESIDENTE y COORDINADOR pueden modificar los miembros.
          </div>
        )}
        ...
        <ul className="mb-4 space-y-2 max-h-64 overflow-y-auto">
          ...
          {members.map((m, idx) => (
            <li key={idx} className="flex justify-between items-center bg-gray-100 px-3 py-2 rounded">
              ...
              {m !== user.username && canEditMembers && (
                <button
